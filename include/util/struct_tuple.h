...
template <size_t I, typename T> struct struct_element;

template <typename T> struct struct_element_count;

template <size_t idx, typename T> const char *getname(const T &);

template <typename T> const char *get_struct_name(const T &);

template <size_t idx, typename T>
typename struct_element<idx, T>::type &get(T &);

template <size_t I, typename T, typename Map> void read_field(T &t, Map &m) {
...
#define DEFINE_GET(T, m, I)                                                    \
  template <> struct struct_element<I, const T> {                              \
    typedef const decltype(T::m) type;                                         \
  };                                                                           \
  template <> struct struct_element<I, T> { typedef decltype(T::m) type; };    \
  template <>                                                                  \
  inline struct_element<I, const T>::type &get<I, const T>(const T &t) {       \
    return t.m;                                                                \
  };                                                                           \
  template <> inline struct_element<I, T>::type &get<I, T>(T & t) {            \
    return t.m;                                                                \
  };                                                                           \
  template <> inline const char *getname<I, T>(const T &) { return #m; };

#define DEFINE_COUNT(T, I)                                                     \
  template <> struct struct_element_count<T> {                                 \
    inline constexpr size_t count() { return I; };                             \
  };

#define DEFINE_NAME(T)                                                         \
  template <> inline const char *get_struct_name<T>(const T &) { return #T; };
